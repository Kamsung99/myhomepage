...
public class BbsDAO {
  ...
  public BbsDTO read(int bbsno){
    ...
    try {
      ...
      if(rs.next()){
        dto = new BbsDTO();
        dto.setBbsno(rs.getInt("bbsno"));
        dto.setWdate(rs.getString("wname"));
        dto.setTitle(rs.getString("title"));
  ...
  public int total(String col, String word){
    int total = 0;
    Connection con = DBOpen.getConnection(); // open()
    PreparedStatement pstmt = null;
    ResultSet rs = null;
    StringBuffer sql = new StringBuffer();
   ...
   public List<BbsDTO> list(Map map){
      ...
      Connection con = DBOpen.getConnection(); // open()
       ...
       int sno = (Integer)map.get("sno"); // 시작행
       int eno = (Integer)map.get("eno");
       
       StringBuffer sql = new StringBuffer();
       sql.append(" SELECT bbsno, wname, title, wdate, grpno, indent, ansnum, wdate ");
       sql.append(" FROM bbs "); 
